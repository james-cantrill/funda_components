...
-- make all five programs visible to user Roger Fitzroger, "login": "opal"

DO $$
DECLARE  _output_json	json;
BEGIN	
	RAISE NOTICE 'Make A Good Program visible to opal. expected result: Success';
...
DECLARE  _output_json	json;
BEGIN	
	RAISE NOTICE 'Make A Very Good Program visible to opal. expected result: Success';
	_output_json := (SELECT * FROM programs_manager_schema.action_change_program_user_visibility ('{"target_login": "opal", "program":"A Very Good Program", "visible":"TRUE", "changing_user_login": "muser"}'));
	RAISE NOTICE 'TEST Result = %', (SELECT _output_json ->> 'result_indicator')::text;
	RAISE NOTICE 'Message = %', (SELECT _output_json ->> 'message')::text;
	RAISE NOTICE '';
END$$;
...
DECLARE  _output_json	json;
BEGIN	
	RAISE NOTICE 'Make Program 3 visible to opal. expected result: Success';
	_output_json := (SELECT * FROM programs_manager_schema.action_change_program_user_visibility ('{"target_login": "opal", "program":"Program 3", "visible":"TRUE", "changing_user_login": "muser"}'));
	RAISE NOTICE 'TEST Result = %', (SELECT _output_json ->> 'result_indicator')::text;
	RAISE NOTICE 'Message = %', (SELECT _output_json ->> 'message')::text;
	RAISE NOTICE '';
END$$;
...
DECLARE  _output_json	json;
BEGIN	
	RAISE NOTICE 'Make Program 4 visible to opal. expected result: Success';
	_output_json := (SELECT * FROM programs_manager_schema.action_change_program_user_visibility ('{"target_login": "opal", "program":"Program 4", "visible":"TRUE", "changing_user_login": "muser"}'));
	RAISE NOTICE 'TEST Result = %', (SELECT _output_json ->> 'result_indicator')::text;
	RAISE NOTICE 'Message = %', (SELECT _output_json ->> 'message')::text;
	RAISE NOTICE '';
END$$;
...
DECLARE  _output_json	json;
BEGIN	
	RAISE NOTICE 'Make Program 5 visible to opal. expected result: Success';
	_output_json := (SELECT * FROM programs_manager_schema.action_change_program_user_visibility ('{"target_login": "opal", "program":"Program 5", "visible":"TRUE", "changing_user_login": "muser"}'));
	RAISE NOTICE 'TEST Result = %', (SELECT _output_json ->> 'result_indicator')::text;
	RAISE NOTICE 'Message = %', (SELECT _output_json ->> 'message')::text;
	RAISE NOTICE '';
END$$;
...
DECLARE  _output_json	json;
BEGIN	
	RAISE NOTICE 'Test 1 use is not autnhorized to list visible programs for a user. expected result: Failure';
	_output_json := (SELECT * FROM programs_manager_schema.action_list_user_visible_programs ('{"login": "jappl", "changing_user_login": "opal"}'));
	RAISE NOTICE 'TEST Result = %', (SELECT _output_json ->> 'result_indicator')::text;
	RAISE NOTICE 'Message = %', (SELECT _output_json ->> 'message')::text;
...
DECLARE  _output_json	json;
BEGIN	
	RAISE NOTICE 'Test 2 list for user jappl who has no programs visible. expected result: Failure';
	_output_json := (SELECT * FROM programs_manager_schema.action_list_user_visible_programs ('{"login": "jappl", "changing_user_login":  "muser"}'));
	RAISE NOTICE 'TEST Result = %', (SELECT _output_json ->> 'result_indicator')::text;
	RAISE NOTICE 'Message = %', (SELECT _output_json ->> 'message')::text;
	RAISE NOTICE '_output_json = %', _output_json;
END$$;
...
DECLARE  _output_json	json;
BEGIN	
	RAISE NOTICE 'Test 3 list for user opal who has five programs visible. expected result: Success';
	_output_json := (SELECT * FROM programs_manager_schema.action_list_user_visible_programs ('{"login": "opal", "changing_user_login":  "muser"}'));
	RAISE NOTICE 'TEST Result = %', (SELECT _output_json ->> 'result_indicator')::text;
	RAISE NOTICE 'Message = %', (SELECT _output_json ->> 'message')::text;
	RAISE NOTICE '_output_json = %', _output_json;
END$$;
